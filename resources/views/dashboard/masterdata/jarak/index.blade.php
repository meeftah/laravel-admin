...
@section('title', 'Daftar Data Jarak')

@section('breadcrumb')
<div class="br-pageheader pd-y-15 pd-l-20">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
        <a class="breadcrumb-item" href="{{ route('dashboard.jarak.index') }}">Data Jarak</a>
    </nav>
</div>
@endsection
...
<div class="row">
    <div class="col-sm-12 col-md-4 mb-5">
        <div class="card">
            <div class="card-body">
                <form action="{{ route("dashboard.jarak.store") }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('jarak') ? 'is-invalid' : '' }}" type="text"
                            name="jarak" placeholder="Masukkan data jarak"
                            value="{{ old('jarak', null) }}">
                        @error('jarak')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <table class="table table-bordered dt-responsive nowrap" data-form="deleteForm"
                        ...
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                @if(auth()->user()->can('jarak_detail') || auth()->user()->can('jarak_ubah')
                                ||
...
<script>
    $(document).ready( function () {
        $('#datatable-jarak').DataTable({
            ...
            ajax: "{{ route('dashboard.jarak.api') }}",
            columns: [
                ...
                @if(auth()->user()->can('jarak_detail') || auth()->user()->can('jarak_ubah') || auth()->user()->can('jarak_hapus'))
