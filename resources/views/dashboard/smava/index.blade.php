...
<div class="br-pageheader pd-y-15 pd-l-20">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
        <a class="breadcrumb-item" href="{{ route('dashboard.smava.index') }}">Data Virtual Account SMAIT</a>
    </nav>
</div>
@endsection
...
<div class="row">
    <div class="col-sm-12 col-md-4 mb-5">
        <div class="card">
            <div class="card-body">
                <form action="{{ route("dashboard.smava.store") }}" method="POST">
                    @csrf
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('smava') ? 'is-invalid' : '' }}" type="text"
                            name="smava" placeholder="Masukkan VA" value="{{ old('smava', null) }}">
                        @error('smava')
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered dt-responsive nowrap" data-form="deleteForm"
                        style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="datatable-smava">
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                <th>KODE NAMA VA</th>
                                <th>STATUS</th>
                                @if(auth()->user()->can('smava_detail') || auth()->user()->can('smava_ubah')
                                ||
                                auth()->user()->can('smava_hapus'))
...
<script>
    $(document).ready( function () {
        $('#datatable-smava').DataTable({
            ...
            ajax: "{{ route('dashboard.smava.api') }}",
            columns: [
                { data: 'id_smava', name: 'id_smava', visible: false },
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable:false, serachable:false },
                { data: 'smava', name: 'smava' },
                { data: 'kode_nama', name: 'kode_nama' },
                { data: 'status', name: 'status' },
                @if(auth()->user()->can('smava_detail') || auth()->user()->can('smava_ubah') || auth()->user()->can('smava_hapus'))
    ...
    $('#delete-btn').click(function(){
        $.ajax({
            ...
            success:function(data){
                ...
                $('#datatable-smava').DataTable().ajax.reload();
