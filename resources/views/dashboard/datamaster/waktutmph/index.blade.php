...
@section('title', 'Waktu Tempuh')

@section('breadcrumb')
<div class="br-pageheader pd-y-15 pd-l-20">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
        <a class="breadcrumb-item" href="{{ route('dashboard.waktutmph.index') }}">Data Waktu Tempuh</a>
    </nav>
</div>
@endsection
...
<div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
    <h4 class="tx-gray-800 mg-b-5">Data Waktu Tempuh</h4>
    <p class="mg-b-0">Master data Waktu Tempuh</p>
</div>
@endsection
...
<div class="row">
    <div class="col-sm-12 col-md-4 mb-5">
        <div class="card">
            <div class="card-body">
                <form action="{{ route("dashboard.waktutmph.store") }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('waktutmph') ? 'is-invalid' : '' }}" type="text"
                            name="waktutmph" placeholder="Masukkan data waktutmph"
                            value="{{ old('waktutmph', null) }}">
                        @error('waktutmph')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered dt-responsive nowrap" data-form="deleteForm"
                        style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="datatable-waktutmph">
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                <th>KODE</th>
                                <th>WAKTU TEMPUH</th>
                                @if(auth()->user()->can('waktutmph_detail') || auth()->user()->can('waktutmph_ubah')
                                ||
                                auth()->user()->can('waktutmph_hapus'))
...
<script>
    $(document).ready( function () {
        $('#datatable-waktutmph').DataTable({
            ...
            ajax: "{{ route('dashboard.waktutmph.api') }}",
            columns: [
                { data: 'id_waktutmph', name: 'id_waktutmph', visible: false },
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable:false, serachable:false },
                { data: 'kode', name: 'kode' },
                { data: 'waktutmph', name: 'waktutmph' },
                @if(auth()->user()->can('waktutmph_detail') || auth()->user()->can('waktutmph_ubah') || auth()->user()->can('waktutmph_hapus'))
    ...
    $('#delete-btn').click(function(){
        $.ajax({
            ...
            success:function(data){
                ...
                $('#datatable-waktutmph').DataTable().ajax.reload();
