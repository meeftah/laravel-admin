...
@section('title', 'Daftar Data Agama')

@section('breadcrumb')
<div class="br-pageheader pd-y-15 pd-l-20">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
        <a class="breadcrumb-item" href="{{ route('dashboard.agama.index') }}">Data Agama</a>
    </nav>
</div>
@endsection
...
<div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
    <h4 class="tx-gray-800 mg-b-5">Data Agama</h4>
    <p class="mg-b-0">Master data Agama</p>
</div>
@endsection
                ...
                <form action="{{ route("dashboard.agama.store") }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('agama') ? 'is-invalid' : '' }}" type="text"
                            name="agama" placeholder="Masukkan data agama"
                            value="{{ old('agama', null) }}">
                        @error('agama')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered dt-responsive nowrap" data-form="deleteForm"
                        style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="datatable-agama">
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                <th>KODE</th>
                                <th>Agama</th>
                                @if(auth()->user()->can('agama_detail') || auth()->user()->can('agama_ubah')
                                ||
                                auth()->user()->can('agama_hapus'))
...
<script>
    $(document).ready( function () {
        $('#datatable-agama').DataTable({
            ...
            ajax: "{{ route('dashboard.agama.api') }}",
            columns: [
                { data: 'id_agama', name: 'id_agama', visible: false },
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable:false, serachable:false },
                { data: 'kode', name: 'kode' },
                { data: 'agama', name: 'agama' },
                @if(auth()->user()->can('agama_detail') || auth()->user()->can('agama_ubah') || auth()->user()->can('agama_hapus'))
    ...
    $('#delete-btn').click(function(){
        $.ajax({
            ...
            success:function(data){
                ...
                $('#datatable-agama').DataTable().ajax.reload();
