...
<div class="br-pageheader pd-y-15 pd-l-20">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
        <a class="breadcrumb-item" href="{{ route('dashboard.va.sd.index') }}">Virtual Account SDIT</a>
...
<div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
    ...
    <p class="mg-b-0">Ubah data Virtual Account SDIT {{ $vasd->va }}</p>
</div>
@endsection
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('dashboard.va.sd.update', $vasd->id_va_sd) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-sm-12 col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('va') ? 'is-invalid' : '' }}" type="text"
                            name="va" placeholder="Masukkan VA" value="{{ old('va', $vasd->va) }}">
                        @error('va')
                ...
                <div class="col-sm-12 col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('kode_nama') ? 'is-invalid' : '' }}" type="text"
                            name="kode_nama" placeholder="Masukkan kode_nama" value="{{ old('kode_nama', $vasd->kode_nama) }}">
                        @error('kode_nama')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="col-sm-12 col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text"
                            name="status" placeholder="Masukkan data status" value="{{ old('status', $vasd->status) }}">
                        @error('status')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
