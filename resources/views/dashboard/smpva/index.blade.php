...
@section('title', 'Data Virtual Account SMPIT')

@section('breadcrumb')
<div class="br-pageheader pd-y-15 pd-l-20">
    <nav class="breadcrumb pd-0 mg-0 tx-12">
        <a class="breadcrumb-item" href="{{ route('dashboard.smpva.index') }}">Data Virtual Account SMPIT</a>
    </nav>
</div>
@endsection
...
<div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
    <h4 class="tx-gray-800 mg-b-5">Data Virtual Account SMPIT</h4>
    <p class="mg-b-0">Master data Virtual Account SMPIT</p>
</div>
@endsection
...
<div class="row">
    <div class="col-sm-12 col-md-4 mb-5">
        <div class="card">
            <div class="card-body">
                <form action="{{ route("dashboard.smpva.store") }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label class="form-control-label">Virtual Account: <span class="tx-danger">*</span></label>
                        <input class="form-control {{ $errors->has('smpva') ? 'is-invalid' : '' }}" type="text"
                            name="smpva" placeholder="Masukkan VA" value="{{ old('smpva', null) }}">
                        @error('smpva')
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered dt-responsive nowrap" data-form="deleteForm"
                        style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="datatable-smpva">
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                <th>KODE NAMA VA</th>
                                <th>STATUS</th>
                                @if(auth()->user()->can('smpva_detail') || auth()->user()->can('smpva_ubah')
                                ||
                                auth()->user()->can('smpva_hapus'))
...
<script>
    $(document).ready( function () {
        $('#datatable-smpva').DataTable({
            ...
            ajax: "{{ route('dashboard.smpva.api') }}",
            columns: [
                { data: 'id_smpva', name: 'id_smpva', visible: false },
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable:false, serachable:false },
                { data: 'smpva', name: 'smpva' },
                { data: 'kode_nama', name: 'kode_nama' },
                { data: 'status', name: 'status' },
                @if(auth()->user()->can('smpva_detail') || auth()->user()->can('smpva_ubah') || auth()->user()->can('smpva_hapus'))
    ...
    $('#delete-btn').click(function(){
        $.ajax({
            ...
            success:function(data){
                ...
                $('#datatable-smpva').DataTable().ajax.reload();
